...
if (Test-Path -LiteralPath $journalPath) {
  ...
  if (($journalKeys -join ',') -cne 'adminSha256,configSha256,kind,phase,taskName,version,workerSid' -or
      ...
      $journal.phase -notin @('intent', 'layout', 'release', 'config', 'task', 'committed')) {
...
function Write-InstallJournal {
  ...
  $record = [ordered]@{
    ...
    configSha256 = $secretConfigSha256; phase = $Phase
  }
...
$phaseOrder = @('intent', 'layout', 'release', 'config', 'task', 'committed')
